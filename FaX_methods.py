...
class MDE_ind():
    ...
    def influence(self, method, X):
        ...
        if self.n == X.shape[1]:
            ...
            is_z = False
        if is_z == False:
            inpA, inpB = self.XZ_A[:, :X.shape[1]], self.XZ_B[:, :X.shape[1]]
        else:
            inpA, inpB = self.XZ_A, self.XZ_B
        Yhat = method.predict_proba(inpA)[:, 0]
        Yhat_ = method.predict_proba(inpB)[:, 0]
...
class MIM():
    ...
    """
    def __init__(self, X, Z, Y, model=LR(max_iter=500)):
        XZ = np.hstack((X, Z))
    ...
    def get_model_coeffs(self, X):
        coeffs = pd.DataFrame({"Feature" : X.columns.tolist(),"Coefficients" : self.model.coef_[0]})
        return coeffs
